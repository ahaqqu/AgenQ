...
![AgenQ mission control — sessions from ZCode and DeepSeek Harness merged into one board ordered by recency (this capture ran a 48h window, and hermes had no activity in it), every row carrying its color-coded harness letter mark (a blue Z for zcode, a violet D for DeepSeek Harness), a recent-activity feed (tool calls, errors, session starts) with a category filter capped at 20 rows — full-width here because nothing failed — and token-burn sparklines against the 200k context cliff on every card; each card shows its ⏱ live duration (session start → last activity) and each run header adds the run's live time next to its totals over main + subagents; the main-session card hangs off the header on the left with its subagent cards beside it, every card carrying a 💬 button into the live conversation; the pill in the top right is amber because the board is frozen for the capture](docs/screenshot.png)
...
- **Token-burn sparklines** — input tokens per request per agent — main sessions and subagents alike — with a dashed marker at the 200K context cliff (the cliff that cost 36M tokens in the run that motivated this tool — see ahaqqu/agentic-project-template#94)
...
![AgenQ live conversation — a DeepSeek Harness session, opened from its Active Now chip's 💬 button and streaming: the header carries the harness mark and the session title, then user prompts, an assistant reply, collapsed thinking rows and tool chips with their final status](docs/screenshot-conversation.png)
...
| `~/.dsh/sessions/<project>/<session-id>/session[.vN].jsonl.zstd` — via the DeepSeek Harness adapter | session titles, model + reasoning effort, per-request token sparklines and totals, todos, the tool trail, the manager→subagent tree (`parentSession` in each subagent's header), failed turn ends and model retries |
