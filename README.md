...
![AgenQ mission control — sessions from ZCode and Hermes merged into one board ordered by recency, every row carrying its color-coded harness letter mark (blue Z = zcode, orange H = hermes), a recent-activity feed (tool calls, errors, session starts) with a category filter capped at 20 rows — full-width when no failures — failure alerts, and token-burn sparklines against the 200k context cliff on every card; each card shows its ⏱ live duration (session start → last activity) and each run header adds the run's live time next to its totals over main + subagents; the main-session card hangs off the header on the left with its subagent cards beside it, every card carrying a 💬 button into the live conversation](docs/screenshot.png)
