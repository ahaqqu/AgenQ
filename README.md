...
![AgenQ mission control — sessions from ZCode and Hermes merged into one board ordered by recency, every row carrying its color-coded harness letter mark (blue Z = zcode, orange H = hermes), a recent-activity feed (tool calls, errors, session starts) with a category filter capped at 20 rows — full-width when no failures — failure alerts, and token-burn sparklines against the 200k context cliff on every card; each run has a header with the session title and totals over main + subagents, the main-session card hanging off it on the left with its subagent cards beside it, every card carrying a 💬 button into the live conversation](docs/screenshot.png)
...
- **Live todos** — each agent's todo list, animated as it progresses; every card shares the same anatomy (stats, sparkline, todos) — the main session card ("main", like a role) sits leftmost in the row, hung off a header that carries the session title plus run totals (sum of in/out/reqs, cache hit, spawned subagent count, how many are running, last activity across the whole tree)
